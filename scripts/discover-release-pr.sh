...
max_attempts=${RELEASE_PR_DISCOVERY_ATTEMPTS:-12}
retry_delay=${RELEASE_PR_DISCOVERY_RETRY_DELAY_SECONDS:-5}
if [[ ! "$max_attempts" =~ ^[1-9][0-9]*$ ||
      ! "$retry_delay" =~ ^[0-9]+$ ||
      "$max_attempts" -gt 60 ||
      "$retry_delay" -gt 60 ]]; then
  echo "Invalid release-PR discovery retry policy" >&2
  exit 2
fi

# A successful force-push can briefly precede the pull-request API's head SHA.
# Retry only that expected convergence (or a just-created PR that is not yet
# listed). Malformed data, API failures, and multiple trusted candidates remain
...
for ((attempt = 1; attempt <= max_attempts; attempt++)); do
  ...
  if [[ "$candidate_count" != 0 && "$candidate_count" != 1 ]]; then
  ...
  if [[ "$candidate_count" == 0 ]]; then
    ...
    echo "Generated release PR is not visible yet; retrying discovery" >&2
    sleep "$retry_delay"
    continue
  fi
  ...
  if [[ -n "$expected_head_sha" && "$head_sha" != "$expected_head_sha" ]]; then
    ...
    echo "Release PR head has not converged to generated commit; retrying discovery" >&2
    sleep "$retry_delay"
    continue
  fi
  break
