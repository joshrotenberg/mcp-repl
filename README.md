...
On macOS or Linux, install the latest prebuilt binary with:

```bash
curl --proto '=https' --tlsv1.2 -LsSf \
  https://github.com/joshrotenberg/mcp-repl/releases/latest/download/mcp-repl-installer.sh | sh
```

The installer is generated by cargo-dist and installs to Cargo's binary
directory. Reading a script before piping it to a shell is the better habit.
On Windows, download the
`mcp-repl-x86_64-pc-windows-msvc.zip` from the
...
Every release also publishes SHA-256 checksums, a CycloneDX SBOM, GitHub
artifact attestations, and BuildKit provenance and SBOM attestations for the
container. See [Verify a downloaded release](https://github.com/joshrotenberg/mcp-repl/blob/main/docs/releases.md#verify-a-downloaded-release)
